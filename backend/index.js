...
app.use(cors());
app.use(express.json());
app.use(express.urlencoded({ extended: true })); // REQUIRED for Mollie webhook

// 🔌 MongoDB setup
const client = new MongoClient(process.env.MONGODB_URI);
let db, orders, chargers;

client.connect().then(() => {
  db = client.db('ev-charging');
  orders = db.collection('orders');
  chargers = db.collection('chargers');
...
app.post('/api/mollie-webhook', async (req, res) => {
  try {
    ...
    if (!mollieRes.ok) {
      ...
      return res.status(500).json({ error: "Mollie fetch failed" });
    }
    ...
    if (!orderId || !ObjectId.isValid(orderId)) {
      console.error("❌ Invalid or missing order ID in metadata:", orderId);
      return res.status(400).json({ error: "Invalid order ID" });
    }
    ...
    console.log(`✅ Webhook: Order ${isPaid ? 'PAID' : isFailed ? 'FAILED' : 'PENDING'}:`, orderId);

    // Optional: Unreserve charger if payment failed
    if (isFailed) {
      const order = await orders.findOne({ _id: new ObjectId(orderId) });
